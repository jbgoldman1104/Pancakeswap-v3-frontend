...
const masterChefFarmCalls = async (farm: SerializedFarm) => {
  ...
  const multiCallChainId = farmFetcher.isTestnet(quoteToken.chainId) ? ChainId.SCROLL_TESTNET : ChainId.BSC
...
export const fetchMasterChefData = async (farms: SerializedFarmConfig[], chainId: number): Promise<any[]> => {
  ...
  const multiCallChainId = farmFetcher.isTestnet(chainId) ? ChainId.SCROLL_TESTNET : ChainId.BSC
